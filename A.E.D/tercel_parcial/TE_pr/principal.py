...
def cargar():
    n = validar(0)
    v = n * [None]
    titulo1 = ("Fundación", "Imperio", "Planeta", "Horizonte", "Océano", "Cielo")
    titulo2 = ("Luz", "Oscuridad", "Felicidad", "Quietud", "Sabiduría", "Maldición")
    for i in range(n):
        cod = str(random.randint(10000, 50000))
        tit = random.choice(titulo1) + " y " + random.choice(titulo2) + " " + str(i+1)
        tip = random.randint(1, 30)
        cos = round(random.uniform(0, 20000), 2)
        v[i] = Publicacion.publicacion(cod, tit, tip, cos)
